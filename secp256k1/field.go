...
func (f *fieldVal) SetBytes(b *[32]byte) *fieldVal {
	...
	// b[28]{0000,0011} b[29]{1111,1111} b[30]{1111,1111} b[31]{1111,1111} => f.n[0]
	f.n[0] = uint32(b[31]) | uint32(b[30])<<8 | uint32(b[29])<<16 |
		(uint32(b[28])&twoBitsMask)<<24

	// b[25]{0000,1111} b[26]{1111,1111} b[27]{1111,1111} b[28]{1111,1100} => f.n[1]
	f.n[1] = uint32(b[28])>>2 | uint32(b[27])<<6 | uint32(b[26])<<14 |
		(uint32(b[25])&fourBitsMask)<<22

	// b[22]{0011,1111} b[23]{1111,1111} b[24]{1111,1111} b[25]{1111,0000} => f.n[2]
	f.n[2] = uint32(b[25])>>4 | uint32(b[24])<<4 | uint32(b[23])<<12 |
		(uint32(b[22])&sixBitsMask)<<20

	// b[19]{1111,1111} b[20]{1111,1111} b[21]{1111,1111} b[22]{1100,0000} => f.n[3]
	f.n[3] = uint32(b[22])>>6 | uint32(b[21])<<2 | uint32(b[20])<<10 |
		uint32(b[19])<<18

	// b[15]{0000,0011} b[16]{1111,1111} b[17]{1111,1111} b[18]{1111,1111} => f.n[4]
	f.n[4] = uint32(b[18]) | uint32(b[17])<<8 | uint32(b[16])<<16 |
		(uint32(b[15])&twoBitsMask)<<24

	// b[12]{0000,1111} b[13]{1111,1111} b[14]{1111,1111} b[15]{1111,1100} => f.n[5]
	f.n[5] = uint32(b[15])>>2 | uint32(b[14])<<6 | uint32(b[13])<<14 |
		(uint32(b[12])&fourBitsMask)<<22

	// b[9]{0011,1111} b[10]{1111,1111} b[11]{1111,1111} b[12]{1111,0000} => f.n[6]
	f.n[6] = uint32(b[12])>>4 | uint32(b[11])<<4 | uint32(b[10])<<12 |
		(uint32(b[9])&sixBitsMask)<<20

	// b[6]{1111,1111} b[7]{1111,1111} b[8]{1111,1111} b[9]{1100,0000} => f.n[7]
	f.n[7] = uint32(b[9])>>6 | uint32(b[8])<<2 | uint32(b[7])<<10 |
		uint32(b[6])<<18

	// b[2]{0000,0011} b[3]{1111,1111} b[4]{1111,1111} b[5]{1111,1111} => f.n[8]
	f.n[8] = uint32(b[5]) | uint32(b[4])<<8 | uint32(b[3])<<16 |
		(uint32(b[2])&twoBitsMask)<<24

	// b[0]{1111,1111} b[1]{1111,1111} b[2]{1111,1100} => f.n[9]
...
func (f *fieldVal) Normalize() *fieldVal {
	...
	t9 := f.n[9]
	m := t9 >> fieldMSBBits // q0 = x/(b^t)
	t9 = t9 & fieldMSBMask  // t9->t0(r0) = x-q0*(b^t)

	// r = r0 + q0*c - q1*(b^t)    q1 = q0*c/(b^t) = 0
	t0 := f.n[0] + m*977
	...
	t7 = t7 & fieldBaseMask
	t9 = (t8 >> fieldBase) + t9 // t9 may overflows right now
	t8 = t8 & fieldBaseMask
	...
	// Ensure the output less than prime(2^256 - 4294968273)
	m = 1
	if t9 == fieldMSBMask {
		m &= 1
	} else {
	...
	// If t9 is still overflow, do it again, it's caused by
	// the carry in the first round.
	if t9>>fieldMSBBits != 0 {
		m |= 1
	} else {
		m |= 0
	}
	// if r >= prime; then r = r - prime
	t0 = t0 + m*977
...
func (f *fieldVal) PutBytes(b *[32]byte) {
	...
	// b[31-28] = f.n[0].Byte0 + f.n[0].Byte1 + f.n[0].Byte2 +
	// 			  f.n[0].Byte3(only 2 bits available) + f.n[1].Byte0(tail 6 bits)
	...
	// b[27-25] = f.n[1].Byte1 + f.n[1].Byte2 + f.n[1].Byte3(tail 4 bits) +
	//            f.n[2].Byte0(head 4 bits)
...
func (f *fieldVal) Add(val *fieldVal) *fieldVal {
	...
	// Each word has 6 overflow bits, it's safe to calculate carryless addition.
	f.n[0] += val.n[0]
	f.n[1] += val.n[1]
	f.n[2] += val.n[2]
	f.n[3] += val.n[3]
...
// the individual words exceeds a max uint32.
//
// **Therefore it is important that the caller ensures no overflows will occur
